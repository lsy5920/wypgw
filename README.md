...
4. 普通前台展示页通过 `src/data/visualAssets.ts` 统一接入 `public/assets/guofeng-ui-20260509/` 下的页面级插图；问云名册、问心考核、云灯留言和问云小院登录页已按 `02-interaction/public-interaction-pages-guofeng-ui.png` 独立重写为互动页画布，不再沿用旧的大标题卡和通用卷轴面板。
...
打开 `/` 可以看到问云派插画山门首屏、核心功能插画卡片、门派精神、名册流程、云灯预览和公告入口。首屏底部的“立派金典、问心考核、云灯留言、问云小院、公告活动”五个入口已居中排列，点击后会跳到对应页面。
...
4. 页面下半部分是“递交名帖”纸卡，可展开或收起；未登录、未通过问心考核或已经递交过名帖时，会显示对应提示和入口。
5. 点击“问云小院”登录或注册账号。
...
1. 打开 `/wenxin-quiz`。
2. 顶部会显示答题进度和当前得分；右侧会显示考核须知和本次状态。
3. 考核共 30 题：单选题 25 题，每题 3 分；多选题 5 题，每题 5 分，总分 100 分。
4. 页面一次只显示一道题，选项会以两列试卷卡展示，选中后会变成深青色。
5. 到最后一题且全部题目都已作答后，点击“交卷”；未登录时可以浏览题目，但需要先登录才能保存成绩。
...
8. 小院总览会用五个统计卡展示名帖、云灯、雅集、问心考核和消息提醒，并在下方展示最近提醒和小院资料卡。
...
2. 前台互动页已按 `02-interaction/public-interaction-pages-guofeng-ui.png` 重写：问云名册是顶部名册墙加下方递帖表单，问心考核是左试卷右须知，云灯留言是左点灯右留言墙，登录页是左山门右登录纸卡。
