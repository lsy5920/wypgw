...
32. 如果需要后台维护归云堂入群二维码，并在名帖审核通过后醒目提示加入归云堂，请执行归云堂二维码设置脚本：

```text
supabase/migrations/20260429173000_guiyuntang_qr_settings.sql
```

33. 在 Supabase 项目设置中找到项目地址和公开匿名密钥。
34. 在项目根目录新建 `.env.local` 文件：
...
注意：当前名册表单以旧名册字段为准，但“公开故事”和“标签”已经合并为“兴趣爱好”；出生年份会在登记入口、本人小院和后台名帖审核中展示，但不会进入公开名册；线下雅集意愿、普通备注、封面和羁绊状态不会在前台登记入口或公开名册中展示。
...
5. 填写审核通过提示文案和保密提醒。
6. 点击“保存归云堂二维码设置”。
7. 打开 `/admin/applications`，把名帖状态改为“已通过”或“已联系”后，详情里会醒目显示归云堂入群二维码。
8. 确认成员已经进群后，把名帖状态改为“已入群”，醒目提示会收起，仍可点击“查看二维码”查看。

注意：归云堂二维码严禁向外部透露。本项目不会把二维码放进 `public` 静态目录，也不会写进 README 或 SQL；二维码只保存在受 Supabase RLS 保护的后台设置表中。
