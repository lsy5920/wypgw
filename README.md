...
34. 如需检查数据库字段是否已经统一，可执行只读检查脚本：

```text
supabase/check_roster_field_unification.sql
```

35. 在 Supabase 项目设置中找到项目地址和公开匿名密钥。
36. 在项目根目录新建 `.env.local` 文件：
...
1. 后台总览：查看待审核数量。
2. 名册管理：默认展示未审核名帖，可按道名、真实姓名、微信号和待审联系方式搜索，可按未审核、资料修改、已审核、全部、暂存、已退派筛选；用户在问云小院申请修改道名或联系方式后，会进入“资料修改”筛选，管理员展开卡片后可同意或拒绝，最后点击“保存名帖”生效；列表先显示道名、申请时间、申请理由、当前状态和最新问心考核分数，点击卡片后再展开详情编辑审核；审核通过但未确认进群时，后台会醒目显示归云堂二维码提示；状态改为已入群后，醒目提示会收起但仍可查看；管理员也可在详情底部彻底删除名帖，删除后不可恢复。
3. 云灯审核：通过或拒绝留言。
4. 公告管理：创建草稿或发布公告。
...
8. 成员进入 `/yard` 时会自动弹出归云堂入群二维码；确认成员已经进群后，把名帖状态改为“已入群”，弹窗不再自动出现，但用户仍可在小院总览里点击“查看二维码”。

注意：归云堂二维码严禁向外部透露。本项目不会把二维码放进 `public` 静态目录，也不会写进 README 或 SQL；二维码只保存在受 Supabase RLS 保护的设置表中，只有管理员和名帖已通过、已联系、已入群的本人可读取。
...
3. 名帖状态还不是“已通过”“已联系”或“已入群”。
4. 当前账号不是掌门或执事，无法读取后台保密设置。

解决方法：

1. 执行 `supabase/migrations/20260429173000_guiyuntang_qr_settings.sql`。
2. 进入 `/admin/settings` 上传归云堂二维码并保存。
3. 回到 `/admin/applications`，把名帖状态改为“已通过”或“已联系”，即可看到醒目入群提示。
4. 成员确认进群后，把状态改为“已入群”，提示会收起，但可点击“查看二维码”。
5. 不要把二维码上传到 `public` 目录，也不要发到前台页面或公开文档。
...
1. 没有执行 `supabase/migrations/20260501110000_unify_roster_profile_fields.sql`，普通成员还没有读取归云堂二维码的 RLS 权限。
2. 名帖状态还不是“已通过”“已联系”或“已入群”。
...
3. 后台把成员名帖状态改为“已通过”或“已联系”。
4. 成员刷新 `/yard`，系统会自动弹出二维码。
5. 如果成员已经被标记为“已入群”，不会自动弹窗，但可在小院总览里点击“查看二维码”。
