...
网站核心气质是：清雅书院、线上山门、清醒温柔、同行自渡、真诚有界。当前版本按 `draw-ui` 技能和用户提供的整站设计图继续全站复刻：当前主视觉使用 `public/assets/guofeng-ui-20260509/` 图片素材包，统一山门、云灯、小院、后台台账、宣纸远山、白底图标和页面级插画，让前台、问云小院和后台都按同一套参考设计稿展现。
...
4. 普通前台展示页通过 `src/data/visualAssets.ts` 统一接入 `public/assets/guofeng-ui-20260509/` 下的页面级插图；问云名册、问云考核、云灯留言和问云小院登录页已按 `02-interaction/public-interaction-pages-guofeng-ui.png` 独立重写为互动页画布，不再沿用旧的大标题卡和通用卷轴面板。
5. 普通前台页面继续使用 `CloudButton`、`BrandMark`、`GeneratedIcon` 和 `StatusNotice` 等轻量组件；四个前台互动页改用 `interaction-reference-page`、金角纸卡、紧凑筛选条、试卷卡、云灯墙和半屏登录山水结构，业务逻辑仍复用原有服务函数。
6. 问云小院采用深青窄侧栏、顶部薄栏、宣纸应用面板、横向水墨页头、统计卡、审核进度、云灯列表、雅集列表和消息时间线；后台七个页面采用 `AdminPageShell`、左台账右详情、浅色纸卡和移动端单栏结构。
...
37. 在 Supabase 项目设置中找到项目地址和公开匿名密钥。
38. 在项目根目录新建 `.env.local` 文件：
...
6. 问云小院包含小院总览、我的资料、我的名帖、我的云灯、我的雅集和消息提醒；桌面端使用左侧深青导航，手机端点击左上角菜单按钮展开同样的页面入口。
7. 本地只想预览 UI 时，可打开 `/?demo-yard=1#/yard` 使用演示资料查看小院总览，不会写入数据库。
8. 小院总览会用五个统计卡展示名帖、云灯、雅集、问云考核和消息提醒，并在下方展示最近提醒和小院资料卡。
...
2. 名册管理：左侧为名帖台账，可按道名、真实姓名、编号和联系方式搜索，也可按未审核、资料修改、已审核、全部、暂存、已退派筛选；右侧为名帖详情，可编辑状态、身份、编号、资料、归云堂入群状态、管理员备注和用户申请修改内容；审核通过但未确认进群时会显示归云堂二维码提示；管理员也可彻底删除名帖，删除后不可恢复。
...
2. 前台互动页已按 `02-interaction/public-interaction-pages-guofeng-ui.png` 重写：问云名册是顶部名册墙加下方递帖表单，问云考核是左试卷右须知加题号导航，云灯留言是左点灯右留言墙，登录页是左山门右登录纸卡。
3. 问云小院已按用户后台设计稿重写为深青窄侧栏、顶部薄栏和宣纸应用面板，小院总览、我的资料、我的名帖、我的云灯、我的雅集和消息提醒六页分别复刻统计卡、资料表单、审核进度、云灯列表、雅集列表和消息时间线。
4. 管理后台采用深色“执事堂”风格，七个页面按后台设计稿重写为固定深青侧栏、编号题签、宣纸台账、低高度数据卡、紧凑表格和手机端单栏布局，强调审核、秩序和危险操作分层。
5. 旧版公共视觉资产、二版插画资产、旧内置生图素材、旧 draw-ui 矢量资产、旧 Logo、旧主图、旧生成脚本和历史导出文件已经从当前工作区清理；当前页面主要使用 `public/assets/guofeng-ui-20260509/` 的背景图、页面插图、装饰纹样和白底国风图标。
6. 公共视觉组件放在 `src/components/`，后续新增普通前台展示页应优先复用 `CloudButton`、`BrandMark`、`GeneratedIcon` 和 `StatusNotice`；后续新增互动页可复用 `global.css` 中的 `interaction-reference-page`、`interaction-corner-card` 和互动页按钮样式；后续新增问云小院页面应优先复用 `YardPageBanner`、`YardPaperCard`、`YardMetricCard`、`YardStatusPill` 和 `YardEmptyBox`；后续新增后台页应优先复用 `src/pages/admin/AdminUi.tsx`。
7. 最新古风国风页面设计稿放在 `design-drafts/guofeng-ui-20260508/`，按前台展示、前台互动、问云小院、后台管理四类保留横版设计总稿；这些设计稿只作为开发参考，不参与网站运行和线上部署。
8. 古风国风 UI 图片素材包放在 `public/assets/guofeng-ui-20260509/`，当前只保留页面实际引用的背景图、页面插图、装饰纹样和 36 个白色底国风线描功能图标；后续页面重做时应优先引用裁切后的单张图片。
9. 动画以轻量、克制、可持续为原则：页面淡入、云雾慢漂、灯火呼吸、卡片微浮和面板展开；系统已尊重浏览器的减少动态偏好。
10. 移动端统一按 785×706 设计档位校准：官网顶部菜单按钮固定在导航栏右侧，小院顶部菜单按钮固定在左上角并打开侧栏抽屉，后台侧栏会切换成顶部横向导航，首页首屏、小院页面和后台表格都避免横向溢出；手机端底部四入口导航已删除，避免遮挡正文。
...
│        ├─ ornaments/             # 宣纸、云山、云纹和竹叶装饰纹样
│        └─ icons/                 # 36 个白色底国风线描功能图标
├─ design-drafts/
│  └─ guofeng-ui-20260508/         # 古风国风 UI 页面设计稿分类目录
...
│  │  ├─ CloudButton.tsx           # 全站统一按钮和链接按钮
│  │  ├─ EmptyState.tsx            # 列表为空时的统一提示组件
│  │  ├─ GeneratedIcon.tsx          # 按设计图绘制的纯代码线描功能图标
│  │  ├─ RouteScrollReset.tsx       # 路由切换后自动回到页面顶部的统一处理组件
│  │  ├─ SiteFooter.tsx            # 前台页脚
│  │  ├─ SiteHeader.tsx            # 前台顶部导航和手机菜单
│  │  ├─ StatusNotice.tsx          # 全站统一状态提示条
│  │  └─ YardGuofengFrame.tsx      # 问云小院复刻设计稿使用的横幅、纸卡、指标卡和状态组件
│  ├─ data/                        # 金典原文入口、导航配置、演示数据、考核题库
│  │  └─ quizContent.ts            # 问云考核题目、答案和分数档说明
│  ├─ hooks/                       # 登录状态和权限判断
│  ├─ layouts/                     # 前台布局、问云小院布局和后台布局
│  ├─ lib/                         # Supabase 客户端、类型、服务函数、校验函数和异常提示工具
│  ├─ pages/                       # 前台页面
│  │  ├─ JoinPage.tsx              # 问云名册页面，展示公开名册并递交名帖
│  │  ├─ WenxinQuizPage.tsx        # 问云考核页面，提交后记录最新成绩
│  │  ├─ CloudLanternPage.tsx      # 云灯留言页面，提交留言并展示云灯墙
│  │  ├─ LoginPage.tsx             # 问云小院登录页面，支持邮箱、旧编号、注册和找回密码
│  │  ├─ admin/                    # 后台管理页面，包含 AdminUi.tsx 复刻后台题签、纸卡和状态章
│  │  └─ yard/                     # 问云小院用户后台页面
│  ├─ styles/                      # 全局样式和动画
│  ├─ tests/                       # 自动测试
│  ├─ App.tsx                      # 全站路由入口
