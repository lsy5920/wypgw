...
1. 后台总览：查看待审核数量。
2. 名册管理：默认展示未审核名帖，可按道名、真实姓名、微信号搜索，可按未审核、已审核、全部、暂存、已退派筛选；列表先显示道名、申请时间、申请理由、当前状态和最新问心考核分数，点击卡片后再展开详情编辑审核；审核通过但未确认进群时，后台会醒目显示归云堂二维码提示；状态改为已入群后，醒目提示会收起但仍可查看；管理员也可在详情底部彻底删除名帖，删除后不可恢复。
3. 云灯审核：通过或拒绝留言。
4. 公告管理：创建草稿或发布公告。
