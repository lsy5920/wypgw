...
│  ├─ wenyun-logo.png              # 历史门派 Logo，当前页面不再直接调用
│  ├─ wenyun-hero.png              # 旧版首页山门主视觉，当前页面不再直接调用
│  ├─ visual/                      # 旧版名门山门视觉资产，当前页面不再直接调用
│     ├─ mountain-gate.svg         # 远山、云海、山门和灯火主视觉
│     ├─ cloud-texture.svg         # 云雾纹理
│     ├─ paper-grain.svg           # 宣纸纹理
│     └─ seal-mark.svg             # 朱砂印章
│  ├─ visual-v2/                   # 网站重构 2.0 插画历史资产，当前页面不再直接调用
...
│  │  ├─ RitualCard.tsx            # 带入场动效的统一信息卡片
│  │  ├─ SceneBanner.tsx           # 每个页面复用的纯代码场景横幅
