...
网站核心气质是：名门山门、清雅江湖、温暖港湾、来去自由、有界有礼。当前版本已经升级为“东方秘境”高端视觉系统，统一前台、问云小院和后台的页面质感，使用真实山门主视觉、流动云雾、宣纸纹理、玉石青、墨色玻璃、朱砂印和鎏金细线形成更强的第一印象。

## 视觉与移动端设计说明

1. 全站视觉系统集中在 `src/styles/global.css`，统一维护色彩、背景、光影、云雾、卡片、工作台、按钮、动效和移动端基础规则。
2. 首页首屏使用 `public/wenyun-hero.png` 作为沉浸式山门背景，并叠加云纹、扫光、暗色遮罩和行动按钮，保证手机端文字仍然清楚。
3. 前台页面统一使用 `PageShell`、`SectionTitle`、`ScrollPanel`、`RitualCard` 和 `CloudButton`，因此金典、名册、云灯、公告、雅集、联系、登录等页面会保持同一套高级质感。
4. 问云小院和管理后台统一使用深色侧栏、移动端横向导航和 `WorkbenchFrame` 工作台外壳，重点保证长期管理时清晰、耐看、好操作。
5. 动效优先使用位移、透明度和缩放，减少卡顿风险；如果用户系统开启“减少动态效果”，页面会自动降低动画强度。
6. 移动端重点适配 320、375、430、768、1024 和 1440 宽度，按钮、表单、弹窗和导航都按小屏优先检查。
...
4. 公共视觉资产放在 `public/visual/`，公共视觉组件放在 `src/components/`，后续新增页面应优先复用 `PageShell`、`GateHero`、`RitualCard` 和 `WorkbenchFrame`。
5. 动画以轻量、克制、可持续为原则：页面淡入、云雾慢漂、灯火呼吸、卡片微浮和卷轴展开；系统已尊重浏览器的减少动态偏好。
...
│  ├─ wenyun-hero.png              # 首页东方秘境沉浸式山门主视觉
│  └─ visual/                      # 新版名门山门视觉资产
