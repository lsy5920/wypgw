...
19. 在 Supabase 项目设置中找到项目地址和公开匿名密钥。
20. 在项目根目录新建 `.env.local` 文件：
...
1. 打开 `/join`。
2. 在页面上方查看公开名册，名册展示道名、江湖名、编号、账号短编号、性别、身份、辈分字、公开地域、封面、羁绊状态、宣言、公开故事、标签、同行期待和入册时间。
3. 可用道名、江湖名或编号搜索，也可以按辈分字、性别和身份筛选名单。
4. 点击“问云小院”登录或注册账号。
5. 回到登记入口填写道名、江湖名、真实姓名、联系方式、公开地域、性别、身份、封面、宣言、公开故事、标签、羁绊状态和同行期待等内容。
...
12. 旧云栖名册导入后，同门可用三位短编号登录；有出生年份时密码为年份，没有年份时密码为短编号本身。

注意：当前名册表单以旧云栖名册字段为准，出生月份、线下雅集意愿和普通备注不会在前台登记入口或公开名册中展示。
...
│     └─ 20260428173000_import_yunqi_legacy_roster.sql # 导入旧云栖名册并创建编号账号
├─ supabase/functions/
...
3. 登录时账号填写短编号，例如 `001`。
4. 密码优先填写出生年份；如果该同门没有年份，就填写短编号本身，例如 `022`。
...
2026-04-28 18:30 【新增】新增旧测试名册和云灯清空 SQL，补充云栖旧名册导入脚本、旧编号登录说明和 README 操作顺序；同步修复三位旧编号密码登录被前端拦截的问题，名册展示和登记说明改为以旧名册字段为准。
