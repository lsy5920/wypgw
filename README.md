...
网站核心气质是：名门山门、清雅江湖、温暖港湾、来去自由、有界有礼。当前版本已经升级为“网站重构 2.0 插画设计系统”，统一前台、问云小院和后台的页面质感，使用山门插画、云灯插画、名册插画、问心卷轴插画、小院插画、后台插画、宣纸纹理、玉石青、朱砂印和鎏金细线形成更强的第一印象。

## 视觉与移动端设计说明

1. 全站视觉系统集中在 `src/styles/global.css`，统一维护色彩、背景、光影、云雾、卡片、工作台、按钮、动效和移动端基础规则。
2. 首页首屏使用 `public/visual-v2/home-illustration-v2.svg` 作为插画主视觉，配合深色山门舞台、云雾纹理、朱砂印和行动按钮，保证手机端文字仍然清楚。
3. 名册、云灯、问心考核、登录小院、小院总览和后台总览都接入 `public/visual-v2/` 下的插画资产，避免页面只有文字和表单。
4. `网站重构2.0/` 文件夹集中保存设计总览图、页面设计图、视觉资产和后续运行截图，方便直接查看本次重构产物。
5. 前台页面统一使用 `PageShell`、`SectionTitle`、`ScrollPanel`、`RitualCard` 和 `CloudButton`，因此金典、名册、云灯、公告、雅集、联系、登录等页面会保持同一套高级质感。
6. 问云小院和管理后台统一使用深色侧栏、移动端横向导航和 `WorkbenchFrame` 工作台外壳，重点保证长期管理时清晰、耐看、好操作。
7. 动效优先使用位移、透明度和缩放，减少卡顿风险；如果用户系统开启“减少动态效果”，页面会自动降低动画强度。
8. 移动端重点适配 320、375、430、768、1024 和 1440 宽度，按钮、表单、弹窗和导航都按小屏优先检查。
...
20. 复制全部 SQL 内容到 Supabase SQL 编辑器并执行，统一名册身份为“掌门、执事、护灯人、同门”，补齐 001 到 021 的出生年份，新增用户资料直接修改和道名、联系方式待审核字段。
...
30. 如果需要启用问心考核，并要求考核合格后才能登记入册，请执行考核门槛脚本：
...
35. 如需检查数据库字段是否已经统一，可执行只读检查脚本：

```text
supabase/check_roster_field_unification.sql
```

36. 在 Supabase 项目设置中找到项目地址和公开匿名密钥。
37. 在项目根目录新建 `.env.local` 文件：
...
打开 `/` 可以看到问云派插画山门首屏、核心功能插画卡片、门派精神、名册流程、云灯预览和公告入口。
...
1. 打开 `/join`。
2. 页面会先显示精美的折叠登记入口，入口内会提示“登录小院、问心合格、递交名帖、执事审核”的完整流程。
3. 公开名册会以一张张资料卡片横向滑动展示，并随机排序；手机端可左右滑动查看。
4. 点击“筛选”按钮后，可用道名、江湖名或编号搜索，也可以按辈分字、性别和身份筛选名单。
5. 点击“问云小院”登录或注册账号。
6. 先打开 `/wenxin-quiz` 完成问心考核。最新一次成绩达到 80 分以上后，名册登记入口才允许提交。
7. 回到登记入口填写道名、江湖名、真实姓名、联系方式、出生年份、所在城市、性别、宣言、兴趣爱好和同行期待等内容。
8. 身份默认是“同门”，用户不能自行更改；如需设置掌门、执事、护灯人，需要管理员在后台调整。
9. 江湖名、所在城市、宣言、兴趣爱好和同行期待会进入公开名册；真实姓名、联系方式和出生年份只给本人和后台管理员核对。
10. 道名必须以“云”字开头，长度为 2 到 3 个字，例如“云初”“云灯”。
11. 勾选认同门规。
12. 点击“提交名册登记”。
13. 登记会进入后台待审核列表，系统会默认生成类似“问云-云-001”的编号。
14. 每个账号只能递交一份名帖；如果已经递交过，页面会指引你去问云小院的“我的资料”修改资料，或到“我的名帖”查看审核状态。
15. 审核状态会回到你的问云小院消息提醒里。
16. 旧名册导入后，同门可用三位短编号登录；有出生年份时密码为年份，没有年份时密码为短编号本身。
...
1. 打开 `/wenxin-quiz`。
2. 先登录问云小院，未登录时页面会直接提示去登录或注册。
3. 考核共 30 题：单选题 25 题，每题 3 分；多选题 5 题，每题 5 分，总分 100 分。
4. 页面一次只显示一道题，作答后点击“下一题”，也可以用题号圆点回看和修改答案。
5. 到最后一题且全部题目都已作答后，点击“交卷并记录成绩”。
6. 90 到 100 分表示深知门风，可登记入册。
7. 80 到 89 分表示基本明规，可登记入册。
8. 70 到 79 分建议执事说明金典重点后，再决定是否入册。
9. 70 分以下建议暂缓入册，重读金典后再考。
10. 系统会保存每次考核记录，小院和后台展示最新一次结果，名册登记也只看最新一次是否合格。
...
5. 我的资料分为账号安全、小院展示资料和名册资料：账号安全可绑定或更换真实邮箱，也可以修改登录密码。
6. 名册资料里，真实姓名、出生年份和身份会以灰色只读方式展示，用户不能自行更改。
...
12. 小院总览会展示最新问心考核分数，是否合格会影响名册登记。
13. 我的名帖、我的云灯只展示状态和备注，不能直接修改审核结果。
14. 我的雅集可以报名或取消报名。
15. 消息提醒会展示站内通知和邮件发送状态。
...
1. 后台总览：查看待审核数量。
2. 名册管理：默认展示未审核名帖，可按道名、真实姓名、微信号和待审联系方式搜索，可按未审核、资料修改、已审核、全部、暂存、已退派筛选；用户在问云小院申请修改道名或联系方式后，会进入“资料修改”筛选，管理员展开卡片后可同意或拒绝，最后点击“保存名帖”生效；列表先显示道名、申请时间、申请理由、当前状态、归云堂入群状态和最新问心考核分数，点击卡片后再展开详情编辑审核；审核通过但未确认进群时，后台会醒目显示归云堂二维码提示；用户点击“我已入群”或管理员勾选“已入归云堂”后，醒目提示会收起但仍可查看；管理员也可在详情底部彻底删除名帖，删除后不可恢复。
3. 云灯审核：通过或拒绝留言。
4. 公告管理：创建草稿或发布公告。
...
7. 站点设置：修改联系山门说明文字，维护归云堂入群二维码，维护 SMTP 邮件服务设置。
...
2. 登录掌门或执事账号。
3. 打开 `/admin/settings`。
4. 在“归云堂入群二维码”区域上传二维码图片。
5. 填写审核通过提示文案和保密提醒。
6. 点击“保存归云堂二维码设置”。
7. 打开 `/admin/applications`，把名帖状态改为“已通过”或“已联系”。
8. 成员进入 `/yard` 时会自动弹出归云堂入群二维码；成员点击弹窗里的“我已入群”后，会写入独立入群字段，弹窗不再自动出现，但用户仍可在小院总览里点击“查看二维码”。
9. 如果成员已经线下确认入群，管理员也可以在名帖详情里勾选“已入归云堂”，这不会改变名帖审核状态。

注意：归云堂二维码严禁向外部透露。本项目不会把二维码放进 `public` 静态目录，也不会写进 README 或 SQL；二维码只保存在受 Supabase RLS 保护的设置表中，只有管理员和名帖已通过、已联系的本人可读取。

### 全站视觉说明

1. 前台官网采用“名门山门”视觉方向，首页首屏必须第一眼看到问云派、山门、门派 Logo 和入册入口。
2. 问云小院采用温暖但严整的个人工作台风格，重点展示个人名帖、云灯、雅集、消息和问心考核状态。
3. 管理后台采用“执事堂”风格，强调审核、秩序和危险操作分层，避免后台页面杂乱。
4. 旧版公共视觉资产放在 `public/visual/`，二版插画资产放在 `public/visual-v2/`，视觉归档和 UI 设计图放在 `网站重构2.0/`。
5. 公共视觉组件放在 `src/components/`，后续新增页面应优先复用 `PageShell`、`GateHero`、`RitualCard` 和 `WorkbenchFrame`。
6. 插画资产由 `scripts/generate-v2-assets.mjs` 统一生成，后续需要重做图片时可以先改脚本，再重新执行生成命令。
7. 动画以轻量、克制、可持续为原则：页面淡入、云雾慢漂、灯火呼吸、卡片微浮和卷轴展开；系统已尊重浏览器的减少动态偏好。
...
│  ├─ wenyun-hero.png              # 旧版首页山门主视觉，保留用于兼容和回溯
│  ├─ visual/                      # 旧版名门山门视觉资产
│     ├─ mountain-gate.svg         # 远山、云海、山门和灯火主视觉
│     ├─ cloud-texture.svg         # 云雾纹理
│     ├─ paper-grain.svg           # 宣纸纹理
│     └─ seal-mark.svg             # 朱砂印章
│  └─ visual-v2/                   # 网站重构 2.0 插画视觉资产
│     ├─ home-illustration-v2.svg  # 首页插画山门主视觉
│     ├─ roster-illustration-v2.svg # 问云名册插画
│     ├─ lantern-illustration-v2.svg # 云灯留言插画
│     ├─ quiz-illustration-v2.svg  # 问心考核卷轴插画
│     ├─ courtyard-illustration-v2.svg # 问云小院插画
...
│  │  ├─ GateHero.tsx              # 东方秘境沉浸式首屏组件
│  │  ├─ LoginRequiredNotice.tsx   # 需要登录时的统一提醒和登录入口
│  │  ├─ PageShell.tsx             # 前台页面统一外壳
│  │  ├─ RitualCard.tsx            # 带入场动效的仪式感宣纸卡片
│  │  └─ WorkbenchFrame.tsx        # 小院和后台高端工作台外壳
│  ├─ data/                        # 金典原文入口、导航配置、演示数据、考核题库
│  │  └─ quizContent.ts            # 问心考核题目、答案和分数档说明
│  ├─ hooks/                       # 登录状态和权限判断
│  ├─ layouts/                     # 前台布局、问云小院布局和后台布局
│  ├─ lib/                         # Supabase 客户端、类型、服务函数、校验函数
│  ├─ pages/                       # 前台页面
│  │  ├─ WenxinQuizPage.tsx        # 问心考核页面，提交后记录最新成绩
│  │  └─ admin/                    # 后台管理页面
│  │  └─ yard/                     # 问云小院用户后台页面
│  ├─ styles/                      # 全局样式和动画
│  ├─ tests/                       # 自动测试
...
├─ 网站重构2.0/
│  ├─ 01-设计总览/                 # 二版整体视觉规范图
│  ├─ 02-页面设计图/               # 首页、小院、后台等页面 UI 设计图
│  ├─ 03-视觉资产/                 # 分类保存背景、纹理、标识、组件和插画
│  └─ 04-运行截图/                 # 本地运行后导出的桌面端和移动端截图
├─ supabase/
...
│     ├─ 20260429110500_wenxin_quiz_gate.sql # 新增问心考核结果表和名册登记合格门槛
...
7. 没有执行 `20260429110500_wenxin_quiz_gate.sql`，导致数据库缺少问心考核结果表或登记门槛策略。
8. 没有先登录问云小院。
9. 最新一次问心考核没有达到 80 分。
...
8. 确认已经登录问云小院。
9. 打开 `/wenxin-quiz` 完成问心考核，并确认最新成绩达到 80 分以上。
10. 确认登记表单已勾选门规确认，且道名符合“云”字开头的规则。

### 名册登记提示先完成问心考核

原因：问云名册已经和问心考核绑定，只有最新一次问心考核达到 80 分以上，才可以提交登记。
...
5. 成员刷新 `/yard`，系统会自动弹出二维码。
6. 如果成员已经确认入群，不会自动弹窗，但可在小院总览里点击“查看二维码”。

### 问心考核结果在小院或后台看不到

原因：没有执行问心考核 SQL，或者当前页面还停留在旧部署版本。
...
原因：名册登记、问心考核提交、云灯留言和雅集报名都需要绑定问云小院账号，未登录时不会直接写入数据。
