...
export function WorkbenchFrame({ children, className = '' }: WorkbenchFrameProps) {
  return (
    <section className={`workbench-surface mx-auto min-h-[calc(100vh-6rem)] max-w-6xl rounded-[1.75rem] border border-[#c9a45c]/20 p-4 shadow-2xl shadow-[#143044]/8 md:p-6 ${className}`}>
      {children}
    </section>
  )
}
