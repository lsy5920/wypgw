...
export type CodeSceneVariant = 'gate' | 'scroll' | 'lantern' | 'courtyard' | 'workbench' | 'map'

// 这个类型描述纯代码场景入参，入参用于切换不同页面需要的图形主题。
interface CodeSceneProps {
  // 场景主题，用于决定山门、灯火、卷轴、小院或后台的视觉重点。
  variant?: CodeSceneVariant
...
export function CodeScene({ variant = 'gate', label, className = '' }: CodeSceneProps) {
  ...
  const generatedScenePath = generatedScenePathByVariant[variant]
