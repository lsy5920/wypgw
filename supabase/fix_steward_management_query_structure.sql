...
begin
  ...
  select
    ...
    coalesce(p.nickname::text, split_part(coalesce(u.email::text, ''), '@', 1), '问云同门')::text as nickname,
...
select
  u.id,
  coalesce(nullif(u.raw_user_meta_data->>'nickname', ''), split_part(u.email::text, '@', 1), '问云掌门'),
  'founder',
  false,
  now(),
  now()
from auth.users u
where lower(u.email::text) = '3199912548@qq.com'
on conflict (id) do update
set role = 'founder',
