...
$legacy$::jsonb) loop
    ...
    if target_application_id is null then
      ...
      values (
        ...
        case when item->>'initial_password' ~ '^\d{4}$' then item->>'initial_password' else null end,
        item->>'gender',
        nullif(item->>'public_region', ''),
      ...
      set user_id = target_user_id,
          ...
          age_range = case when item->>'initial_password' ~ '^\d{4}$' then item->>'initial_password' else public.join_applications.age_range end,
          gender = item->>'gender',
